...
const CODES = {
  ...
  CM205: { tier: 'structural', section: '§6', message: 'file ends inside a multi-line string', fix: 'close the string — the delimiter swallows the rest of the file, so every cm: annotation below this line is not read and the PreToolUse hook injects nothing for it. If there is nothing to close because the delimiter is inside a heredoc or a raw string, which §6 does not model, say so with: cm:ignore CM205 — <why this is not a string>' },
  CM204: { tier: 'grammar', section: '§4', message: 'annotation overflows the one line it may wrap onto', fix: 'a wrap is one line, not a paragraph (§4) — the lines past it are ordinary comments, so `cm ls`, `cm impact` and the PreToolUse hook never see them and the annotation reaches its reader ending mid-clause. Reword so the rule and its consequence fit the annotation and its one wrap, or split it into two annotations where it carries two rules. If the lines below are orientation prose rather than this annotation running on — a module header is the usual case (§4.1) — move the annotation below them instead, which leaves the prose where it belongs. Do not insert a blank line to silence this: it ends the run without shortening the annotation, and in a header it splits the run so the tail becomes CM001' },
};
