...
const CODES = {
  ...
  CM204: { tier: 'structural', section: '§4', message: 'annotation overflows the one line it may wrap onto', fix: 'a wrap is one line, not a paragraph (§4) — the lines past it are ordinary comments, so `cm ls`, `cm impact` and the PreToolUse hook never see them and the annotation reaches its reader ending mid-clause. Keep the rule and its consequence on the annotation and its wrap; move orientation prose to the module header (§4.1) and the incident retold to the tracker, which already holds it' },
};
