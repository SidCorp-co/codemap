...
export const analyzeCases = [
  ...
  {
    name: 'ts: a backtick in a regex literal does not make the next template a comment (ISS-69)',
    file: 'regex-backtick.ts',
    src: [
      'const a = source.split(/`/);',
      'const one = [`// ${real}`, false];',
    ].join('\n'),
    codes: [],
    annotations: [],
  },
  {
    name: 'ts: a backtick in a regex literal does not open a block in the next template (ISS-69)',
    file: 'regex-backtick-block.ts',
    src: [
  ...
  {
    name: 'ts: a quote in a regex literal no longer swallows the annotation after it (ISS-69)',
    file: 'regex-quote.ts',
    src: [
      "const out = s.replace(/'/g, '');",
      "const two = s.replace(/'/g, ''); // cm:guard the apostrophe above is a regex, never a string",
    ].join('\n'),
    codes: [],
    annotations: ['guard'],
  },
  {
    name: 'ts: a regex literal opening a line is read off the line before it (ISS-69)',
    file: 'regex-newline.ts',
    src: [
      'const re =',
      '  /`/;',
      'const t = `// ${real}`;',
  ...
  {
    name: 'ts: a postfix increment is division, not a regex (ISS-69)',
    file: 'regex-postfix-inc.ts',
    src: 'const q = value++ / divisor; // cm:guard the second + is not a regex position',
    codes: [],
    annotations: ['guard'],
  },
  {
    name: 'ts: a postfix decrement is division, not a regex (ISS-69)',
    file: 'regex-postfix-dec.ts',
    src: 'const q = value-- / divisor; // cm:guard the second - is not a regex position',
    codes: [],
    annotations: ['guard'],
  },
  ...
  {
    name: 'ts: division after an identifier still finds the comment after it (ISS-69)',
    file: 'regex-division.ts',
    src: 'const q = a / b; // cm:guard plain division is untouched',
    codes: [],
    annotations: ['guard'],
  },
  {
    name: 'ts: a slash whose literal does not close on the line is left as code (ISS-69)',
    file: 'regex-unclosed.ts',
    src: 'const q = (a / b; // cm:guard an unclosed regex declines and the scan is unchanged',
    codes: [],
    annotations: ['guard'],
  },
  {
    name: 'ts: a slash inside a character class does not end the regex literal (ISS-69)',
    file: 'regex-class.ts',
    src: 'const re = /[/`]/; // cm:guard the class holds both a slash and a backtick',
    codes: [],
    annotations: ['guard'],
  },
  {
    name: 'go: a language with no regex literals does not consume a slash as one (ISS-69)',
    file: 'regex-none.go',
    src: 'q := a / b // cm:guard go sets no regexLiteral and scans exactly as it did',
    codes: [],
    annotations: ['guard'],
  },
  {
