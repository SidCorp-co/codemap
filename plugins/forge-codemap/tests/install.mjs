...
function hookCommand(pluginRoot, name) {
  return join(pluginRoot, 'scripts', name);
}
...
export function installCases(pluginRoot, check) {
  const pluginCm = join(pluginRoot, 'scripts', 'cm.mjs');
  const roots = [];
  try {
    const root = makeRepo();
    roots.push(root);
    ...
    const src = readdirSync(join(pluginRoot, 'scripts', 'lib')).filter((f) => f.endsWith('.mjs')).sort();
