...
| | How | Cost |
|---|---|---|
| **vendored (recommended)** | `cm install`, commit `.forge/codemap/`, CI runs `.forge/codemap/cm verify` | the gate is deterministic and reviewable; you must bump it (`cm install --upgrade`, or the scheduled PR job in `agent-setup/codemap-upgrade.yml`) |
| **pinned clone** | `git clone --branch codemap-v<x.y.z> … && node …/scripts/cm.mjs verify` in CI | same determinism, nothing committed — but nothing local tells you the tag is old, so pin deliberately and revisit it |
...
manager. `agent-setup/prompt.md` is a fetchable bootstrap for an agent setting a repo up from nothing;
